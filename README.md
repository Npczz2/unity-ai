...
The enemies also use **Unity NavMesh** to create walking paths through the map.

# Enemy behavior
Here are all the enemy behaviors implemented on the game.

## Patrol State
The enemy default state is the **patrol state.** On this mode, he has to reach the current point declared on an array of **Vector3 positions.** The guard keeps repeating the path until something changes his state.  
...
void OnCollisionEnter(Collision col)
{
    if (col.gameObject.tag == "Floor")
    {
        ...
        {
            //Check for any enemies within 15 units of distance
...
## Chase State
In the chase state, the enemy **destination** is set to the player's current position. There are two situations that can trigger the **chase state:**
...
- The player is **heard by the guard.** This happens when the player walks near the enemy. Inside the guard script there is a specified distance that is considered for this to happen.
...
## Hard Search State
If the player is out of enemy sight, the guards enter in the **hard search state.** When chasing, the enemies store the *last player known position* every 0.2 seconds. With this information, he can go to that last position when he enters this new state. When the enemy reaches the last player known position, he starts to look around and walk near the spot trying to find the player.  
When a guard enters this state, he also warns other guards near him about the situation, making them enter on a hard search state too.  
  
![HSState](https://github.com/Npczz2/unity-ai/blob/main/GIFs/Hard%20Search%20Gif.gif)

# Prototype
The game prototype can be played in browser: https://npcproductions.itch.io/sck981-demo
